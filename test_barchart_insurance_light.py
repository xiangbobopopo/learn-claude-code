"""
Test script for the Insurance Barchart Builder Agent (Light Version)
"""

from barchart_insurance_agent_light import run_insurance_barchart_agent
import json
...
Commercial Premiums, 2900000"""
    ...
    if result.startswith("{"):
        ...
        print(f"✅ SUCCESS: Generated chart config with {len(config['data']['categories'])} categories")
        print(f"   Title: {config['title']['text']}")
        print(f"   Orientation: {config['orientation']}")
        print(f"   Color: {config['styling']['primary_color']}")
        return True
    else:
...
Commercial Claims, 1500000"""
    ...
    if result.startswith("{"):
        ...
        print(f"✅ SUCCESS: Generated chart config with {len(config['data']['categories'])} categories")
        print(f"   Title: {config['title']['text']}")
        print(f"   Color scheme: {config['styling']['primary_color']} (should be risk red)")
        return True
    else:
        print(f"❌ FAILED: {result}")
        return False
...
def test_json_format():
    ...
    if result.startswith("{"):
        ...
        print(f"✅ SUCCESS: Generated chart config with {len(config['data']['categories'])} categories")
        print(f"   Title: {config['title']['text']}")
        print(f"   Has temporal data: {config['insurance_context']['has_periods']}")
        print(f"   Color: {config['styling']['primary_color']} (should be profit green)")
        return True
    else:
        print(f"❌ FAILED: {result}")
        return False
...
West, 3500000"""
    ...
    if result.startswith("{"):
        ...
        print(f"✅ SUCCESS: Generated chart config with {len(config['data']['categories'])} categories")
        print(f"   Title: {config['title']['text']}")
        print(f"   Value range: ${config['insurance_context']['value_range']['min']:,.0f} - ${config['insurance_context']['value_range']['max']:,.0f}")
        return True
    else:
        print(f"❌ FAILED: {result}")
        return False
...
def test_large_dataset():
    ...
    data = """Very Long Category Name Insurance Product Alpha, 1250000
...
Extended Category Insurance Product Kappa, 1340000"""
    ...
    if result.startswith("{"):
        config = json.loads(result)
        print(f"✅ SUCCESS: Generated chart config with {len(config['data']['categories'])} categories")
        print(f"   Orientation: {config['orientation']} (should be horizontal for long categories)")
        print(f"   Title: {config['title']['text']}")
        return True
    else:
        print(f"❌ FAILED: {result}")
...
def test_auto_detection():
    ...
    print("\n" + "=" * 60)
    print("TEST 7: Auto-Detection of Data Type")
    print("=" * 60)
    
    # This should auto-detect as "premium" data
    data = """Auto Premiums, 2500000
Home Premiums, 1800000
Life Premiums, 3200000"""
    
    result = run_insurance_barchart_agent(data)  # No data_type specified
    
    if result.startswith("{"):
        config = json.loads(result)
        print(f"✅ SUCCESS: Auto-detected data type")
        print(f"   Detected types: {config['insurance_context']['data_types']}")
        print(f"   Title: {config['title']['text']}")
        return True
    else:
        print(f"❌ FAILED: {result}")
...
if __name__ == "__main__":
    print("Testing Insurance Barchart Builder Agent (Light Version)")
    print("This will generate bar chart configurations for various insurance data scenarios")
    
    tests = [
        test_basic_premium_data,
        test_claims_data,
        test_json_format,
        test_regional_data,
        test_text_format,
    ...
    for test in tests:
        try:
            ...
            print(f"❌ FAILED: Exception in {test.__name__}: {e}")
    ...
    print("✓ Comprehensive chart configuration output")
    print("✓ Various insurance data types (premiums, claims, policies, etc.)")
    print("✓ Value formatting and statistical analysis")